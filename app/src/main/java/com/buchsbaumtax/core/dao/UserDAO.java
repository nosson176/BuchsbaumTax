...
public interface UserDAO {
    ...
    @GetGeneratedKeys
    @SqlUpdate("INSERT INTO users (first_name, last_name, username, user_type, password, phone_number, send_login_notifications, notify_of_logins, seconds_in_day, allow_texting, selectable, email) VALUES (:firstName, :lastName, :username, :userType, :password, :phoneNumber, :sendLoginNotifications, :notifyOfLogins, :secondsInDay, :allowTexting, :selectable, :email)")
    int create(@BindBean UserCreate userCreate);

    @SqlUpdate("UPDATE users SET first_name = :firstName, last_name = :lastName, username = :username, phone_number = :phoneNumber, send_login_notifications = :sendLoginNotifications, notify_of_logins = :notifyOfLogins, seconds_in_day = :secondsInDay, allow_texting = :allowTexting, selectable = :selectable, user_type = :userType,email = :email, updated = now() WHERE id = :id")
